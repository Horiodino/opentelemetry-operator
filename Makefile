...
# Tool versions for supply chain security
# renovate: datasource=github-releases depName=sigstore/cosign
...
SLSA_VERIFIER ?= $(LOCALBIN)/slsa-verifier
...
# Download syft locally if necessary
.PHONY: syft
syft: $(LOCALBIN)
	@{ \
	set -e ;\
	if [ -x "$(SYFT)" ] && "$(SYFT)" version 2>/dev/null | grep -q "$(SYFT_VERSION)"; then exit 0; fi ;\
	curl -sSfL "https://raw.githubusercontent.com/anchore/syft/main/install.sh" | sh -s -- -b "$(LOCALBIN)" "$(SYFT_VERSION)" ;\
	}

# Download grype locally if necessary
.PHONY: grype
grype: $(LOCALBIN)
	@{ \
	set -e ;\
	if [ -x "$(GRYPE)" ] && "$(GRYPE)" version 2>/dev/null | grep -q "$(GRYPE_VERSION)"; then exit 0; fi ;\
	curl -sSfL "https://raw.githubusercontent.com/anchore/grype/main/install.sh" | sh -s -- -b "$(LOCALBIN)" "$(GRYPE_VERSION)" ;\
	}

# Download slsa-verifier locally if necessary
.PHONY: slsa-verifier
slsa-verifier: $(LOCALBIN)
	@{ \
	set -e ;\
	if [ -x "$(SLSA_VERIFIER)" ] && "$(SLSA_VERIFIER)" version 2>/dev/null | grep -q "$(SLSA_VERIFIER_VERSION)"; then exit 0; fi ;\
	OS=$(shell go env GOOS) && ARCH=$(shell go env GOARCH) ;\
	curl -sSfL "https://github.com/slsa-framework/slsa-verifier/releases/download/$(SLSA_VERIFIER_VERSION)/slsa-verifier-$${OS}-$${ARCH}" -o "$(SLSA_VERIFIER)" ;\
	chmod +x "$(SLSA_VERIFIER)" ;\
	}

# Download SLSA provenance attestation for a released image.
# Usage: make provenance-download IMAGE=ghcr.io/... DIGEST=sha256:... PROVENANCE_FILE=output.intoto.jsonl
.PHONY: provenance-download
provenance-download: cosign
ifndef IMAGE
	$(error IMAGE is not set)
endif
ifndef DIGEST
	$(error DIGEST is not set)
endif
ifndef PROVENANCE_FILE
	$(error PROVENANCE_FILE is not set)
endif
	$(COSIGN) download attestation "$(IMAGE)@$(DIGEST)" \
		--predicate-type slsaprovenance > "$(PROVENANCE_FILE)"

# Generate SPDX and CycloneDX SBOMs for a container image using syft.
# Outputs: opentelemetry-operator-<tag>.sbom.spdx.json and opentelemetry-operator-<tag>.sbom.cyclonedx.json
.PHONY: sbom-generate
sbom-generate: syft
ifndef IMAGE
	$(error IMAGE is not set. Usage: make sbom-generate IMAGE=<image> DIGEST=<digest> RELEASE_TAG=<tag>)
endif
ifndef DIGEST
	$(error DIGEST is not set. Usage: make sbom-generate IMAGE=<image> DIGEST=<digest> RELEASE_TAG=<tag>)
endif
ifndef RELEASE_TAG
	$(error RELEASE_TAG is not set. Usage: make sbom-generate IMAGE=<image> DIGEST=<digest> RELEASE_TAG=<tag>)
endif
	$(SYFT) "$(IMAGE)@$(DIGEST)" -o "spdx-json=opentelemetry-operator-$(RELEASE_TAG).sbom.spdx.json"
	$(SYFT) "$(IMAGE)@$(DIGEST)" -o "cyclonedx-json=opentelemetry-operator-$(RELEASE_TAG).sbom.cyclonedx.json"

# Attest SPDX and CycloneDX SBOMs to a container image using cosign.
# Usage: make sbom-attest IMAGE=ghcr.io/... DIGEST=sha256:... RELEASE_TAG=v0.x.y
# Requires sbom-generate to have been run first.
.PHONY: sbom-attest
sbom-attest: cosign
ifndef IMAGE
	$(error IMAGE is not set. Usage: make sbom-attest IMAGE=<image> DIGEST=<digest> RELEASE_TAG=<tag>)
endif
ifndef DIGEST
	$(error DIGEST is not set. Usage: make sbom-attest IMAGE=<image> DIGEST=<digest> RELEASE_TAG=<tag>)
endif
ifndef RELEASE_TAG
	$(error RELEASE_TAG is not set. Usage: make sbom-attest IMAGE=<image> DIGEST=<digest> RELEASE_TAG=<tag>)
endif
	$(COSIGN) attest --yes --predicate "opentelemetry-operator-$(RELEASE_TAG).sbom.spdx.json" --type spdxjson "$(IMAGE)@$(DIGEST)"
	$(COSIGN) attest --yes --predicate "opentelemetry-operator-$(RELEASE_TAG).sbom.cyclonedx.json" --type cyclonedx "$(IMAGE)@$(DIGEST)"

# Scan the SPDX SBOM for vulnerabilities using grype.
# Usage: make sbom-scan RELEASE_TAG=v0.x.y
# Requires sbom-generate to have been run first.
.PHONY: sbom-scan
sbom-scan: grype
ifndef RELEASE_TAG
	$(error RELEASE_TAG is not set. Usage: make sbom-scan RELEASE_TAG=<tag>)
endif
	$(GRYPE) "sbom:opentelemetry-operator-$(RELEASE_TAG).sbom.spdx.json" --output table

# Generate and attest SBOMs. Composite target for the SBOM generation + attestation workflow.
# Usage: make security-artifacts IMAGE=ghcr.io/... DIGEST=sha256:... RELEASE_TAG=v0.x.y
.PHONY: security-artifacts
security-artifacts: sbom-generate sbom-attest

# Verify the keyless cosign signature and SLSA provenance of a released image.
# Usage: make supply-chain-verify IMAGE=ghcr.io/... DIGEST=sha256:... GITHUB_REPOSITORY=open-telemetry/opentelemetry-operator
.PHONY: supply-chain-verify
supply-chain-verify: cosign slsa-verifier
ifndef IMAGE
	$(error IMAGE is not set)
endif
ifndef DIGEST
...
ifndef GITHUB_REPOSITORY
	$(error GITHUB_REPOSITORY is not set, e.g. open-telemetry/opentelemetry-operator)
endif
	$(COSIGN) verify "$(IMAGE)@$(DIGEST)" \
		--certificate-identity-regexp "^https://github.com/$(GITHUB_REPOSITORY)/.github/workflows/publish-images.yaml@refs/tags/v.+$$" \
		--certificate-oidc-issuer "https://token.actions.githubusercontent.com"
	$(SLSA_VERIFIER) verify-image "$(IMAGE)@$(DIGEST)" \
